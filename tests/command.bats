...
@test "Run command" {
  ...
  stub docker \
    "run -it --rm --volume $PWD:/app --workdir /app --env BUILDKITE_JOB_ID  --env BUILDKITE_BUILD_ID --env BUILDKITE_AGENT_ACCESS_TOKEN --volume /buildkite-agent:/usr/bin/buildkite-agent image:tag bash -ce 'command1 \"a string\"' : echo ran command in docker"
...
@test "Run command without a workdir should not fail" {
  ...
  stub docker \
    "run -it --rm --volume $PWD:/workdir --workdir /workdir --env BUILDKITE_JOB_ID  --env BUILDKITE_BUILD_ID --env BUILDKITE_AGENT_ACCESS_TOKEN --volume /buildkite-agent:/usr/bin/buildkite-agent image:tag bash -ce 'command1 \"a string\"' : echo ran command in docker"
...
@test "Pull image first before running the command with mount-buildkite-agent disabled" {
  ...
  stub docker \
    ...
    "run -it --rm --volume $PWD:/app --workdir /app image:tag bash -ce 'pwd' : echo ran command in docker"
...
@test "Runs command with volumes" {
  ...
  stub docker \
    "run -it --rm --volume $PWD:/app --workdir /app --volume /hello:/hello-world --volume /var/run/docker.sock:/var/run/docker.sock image:tag bash -ce 'echo hello world; pwd' : echo ran command in docker"
...
@test "Runs command with environment" {
  ...
  stub docker \
    "run -it --rm --volume $PWD:/app --workdir /app --env MY_TAG=value --env ANOTHER_TAG=llamas image:tag bash -ce 'echo hello world' : echo ran command in docker"
...
@test "Runs command with user" {
  ...
  stub docker \
    "run -it --rm --volume $PWD:/app --workdir /app -u foo image:tag bash -ce 'echo hello world' : echo ran command in docker"
...
@test "Runs command with additional groups" {
  ...
  stub docker \
    "run -it --rm --volume $PWD:/app --workdir /app --group-add foo --group-add bar image:tag bash -ce 'echo hello world' : echo ran command in docker"
...
@test "Runs command with network that doesn't exist" {
  ...
  stub docker \
    ...
    "run -it --rm --volume $PWD:/app --workdir /app --network foo image:tag bash -ce 'echo hello world' : echo ran command in docker"
...
@test "Runs with debug mode" {
  ...
  stub docker \
    "run -it --rm --volume $PWD:/app --workdir /app image:tag bash -ce 'echo hello world' : echo ran command in docker"
